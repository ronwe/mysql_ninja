...
/*分析from 的表名*/
function getAfffectTableName(type , sql ,default_db){
	sql = sql.toLowerCase()
	let sql_part
	switch(type){
		case 'select':
...
function _extra_value(inserted , field_len){
	...
	if (row_nums > 1  && values.length % field_len === 0){
		//多值
		let all_values = []
...
function getUseDB(sql){
	return sql.replace(/^use\b/,'').trim().toLowerCase()	
}

function wrap(sql,_default_db){
	let id = Lib.md5(sql)
	return {
		id : id,
...
exports.getInsertRecord = function(sql ,tables){
	return _get_newvalue(sql ,tables)
}

exports.getTableNames = function(sql ,type , default_db){
	return getAfffectTableName(type || 'select',sql , default_db)
}
