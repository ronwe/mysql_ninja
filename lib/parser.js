function _get_query_com(sql,type , default_db , CacheTables){
	sql = sql.toLowerCase()
	let tables = _get_afffect_tableName(type ,sql , default_db)	
		,sql_part = sql.split(/where/i,2)
		,fields = []
		,table_name = []
		,sql_query
	tables.forEach( table => {
		let _c = CacheTables[table.name]
		if (!_c) return
		table_name.push(table.name)
		fields.push(_c.id)
	})

	if (!fields.length ) return false
	sql_query = '/*updatemark*/select ' + fields.join(',') + ' from ' + table_name.join(',')
	if (sql_part.length > 1) sql_query += ' where ' + sql_part[1]
	
	
	return sql_query.trim()	
}

function _get_select_fields(sql ,tables ){
	let table_alias_hash = {}
		,ret_all = [] 
		,default_table

	sql = sql.toLowerCase()
	let sql_part = sql.split(' from ')[0]
	sql_part = sql_part.split('select ')[1].trim()

	table_check = tables.every(t => {
		table_alias_hash[t.alias] =  t.name
		return true
	})
	default_table = tables[tables.length-1].name

	sql_part.split(',').forEach( part => {
		part = part.trim()
			.replace(' as ',' ')
			.replace(/ +/,' ')
			.split(' ')
		let field_part = part[0].split('.')		
			,table_name
			,field_name
			,filed_alias = part[1] || ''

		if (field_part.length === 2){
			table_name = field_part[0]	
			table_name = table_alias_hash[table_name] || table_name
...
/*
* 获得update语句的更新值
* update table set fielda = val1 where ....*/

function _get_updated(sql,table){
	if (!table || !table.length) return false
	table = table[0]	

	let table_name = table.name 
		,sql_part = sql.slice(sql.indexOf(table_name) + table_name.length + 1)
		,_seek_reg = /("|').*?\1|\,/i			
		,seeked
		,ret = {}
		
	sql_part = sql_part.split(/where/i,1)[0].trim()	
	if (!/^set\b/.test(sql_part)) return false

	sql_part = sql_part.replace(/^set +/i,'')

	function splitFieldValue(part){
		part = part.toString().trim().replace(/^,/,'').replace(/,$/,'').trim()
		if (!part) return false
		
		let opchar = part.indexOf('=')
		if (opchar === -1) return false
		let field = part.slice(0,opchar)
			,val = part.slice(opchar + 1)

		ret[field] = tryConvertStr(val)
	}

	while (seeked = _seek_reg.exec(sql_part)){
		splitFieldValue(sql_part.slice(0 , seeked.index + seeked[0].length) )
		sql_part = sql_part.slice(seeked.index + seeked[0].length)
	}
	return ret			
}
/*
...
exports.getUpdatedField = _get_updated 
exports.getAfffectTableName = _get_afffect_tableName 
exports.getSelectFields = _get_select_fields
exports.getQueryCom = _get_query_com
