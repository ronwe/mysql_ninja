...
//检查影响范围
function dataChange(query, prev){
	if (!query.id) return false
	analytic_child.send({affect:{
...
function fillCache(query , result ){
	...
	query.key_where = {}
	//console.log(query.influence,result.columns)
	let influence = query.influence

	for (let _i = 0 , _j = result.columns.length ; _i < _j ;_i++){
		let col_name = result.columns[_i]
		if (col_name in influence){
			query.key_where[influence[col_name].table + ':' + influence[col_name].name] = result.columns_values[_i]
		}
	}
